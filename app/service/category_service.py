...
async def create_category_service(category: CategoryNewSchema, db: AsyncSession) -> Category:
    """
...
async def add_client_to_category_service(category_client: CategoryClientSchema, db: AsyncSession) -> CategoryClient:
    """
    ...
    db_category = db.query(Category).filter(Category.id == category_client.id_category).first()
    ...
    if db_client and db_category:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Client or Category not found")
